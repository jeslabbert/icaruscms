...
{
    protected $signature = 'tenant:create {name} {username} {email}';
    protected $description = 'Creates a tenant with the provided names and email address e.g. php artisan tenant:create taskmule taskadmin stefan@example.com';
    public function handle()
    {
        ...
        $password = str_random();
        $this->addAdmin($name, $username, $email, $password);
    ...
    private function addAdmin($name, $email, $password)
    {
        ...
        $admin = User::create(['username' => $username, 'name' => $name, 'first_name' => $firstname, 'last_name' => $lastname, 'email' => $email, 'password' => Hash::make($password)]);
        return $admin;
    }
}